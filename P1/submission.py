...
def hill_climbing_search(park_size, playgrounds, restrooms):
    ...
    for i in range(100):  # 最大迭代次數
        for idx, restroom_position in enumerate(restrooms):  # 迭代廁所位置 !!!程式結果看起來沒做到

            #print(restrooms)      
            #if restroom_position == best_position:  # 確保所選的廁所位置不在遊樂場上 #這行不是這個意思 /這裡是廁所=廁所 /應該比對
                #continue
            ...
            for move in [(0, 1), (0, -1), (1, 0), (-1, 0)]:  # 上下左右移動
                ...
                if 0 <= new_position[0] < park_size[0] and 0 <= new_position[1] < park_size[1] and new_position not in playgrounds:  
                    ...
                    if new_cost < best_cost:
                        ...
                        restrooms.append(new_position)
                #上面的code只會讓程式跑一次就結束，但需要的是繼續跑
                         
                         #1.tree-insert) 從[0,1]拓展到周圍的點 2.iterative 改new position
